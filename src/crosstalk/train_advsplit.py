...
def run_experiment(config):
    ...
    Uses adversarial validation splitting instead of random splits.
    """
    # --- Setup ---
    ...
    # Load additional data needed for adversarial splitting
    data_df = pd.read_parquet(config['DATA_PATH'])
    if config.get('MAX_ROWS'):
        data_df = data_df.head(config.get('MAX_ROWS'))
    
    groups = data_df['DEL_ID'].values
    adversarial_scores = data_df['adversarial_score'].values
    ...
    if config.get('CLUSTER_SPLIT', {}).get('enabled', False):
        ...
        cluster_ids = data_df['cluster_id'].values
    ...
    # 2. Adversarial Split Data (Train / Validation / Test)
    if config.get('HYPERPARAM_TUNING', {}).get('enabled', False):
        print("\n[2/6] Splitting data using adversarial scores into train/validation (for tuning) and test sets...")
    else:
        print("\n[2/6] Splitting data using adversarial scores into train and test sets...")
    
    # Calculate adversarial score threshold for test set selection
    test_size = config['TEST_SIZE']
    adversarial_threshold = np.percentile(adversarial_scores, (1 - test_size) * 100)
    
    print(f"Using adversarial score threshold: {adversarial_threshold:.6f}")
    print(f"This selects the top {test_size*100:.1f}% most test-like samples for validation")
    
    # Create adversarial-based split
    test_mask = adversarial_scores >= adversarial_threshold
    train_val_mask = ~test_mask
    
    test_idx = np.where(test_mask)[0]
    train_val_idx = np.where(train_val_mask)[0]
    
    print(f"Adversarial split: {len(train_val_idx)} train+val samples, {len(test_idx)} test samples")
    print(f"Test set adversarial score range: [{adversarial_scores[test_idx].min():.6f}, {adversarial_scores[test_idx].max():.6f}]")
    ...
    print("\n[6/6] Evaluating model on adversarial validation set...")
    eval.evaluate_and_save_results(final_model, X_test_final, y_test, output_dir, result_name='adversarial_test')
