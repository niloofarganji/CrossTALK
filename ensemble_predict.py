...
def run_stacked_prediction():
    ...
    # --- 2. Generate Predictions on New, Unseen Data ---
    print("\n--- Phase 2: Generating Predictions on New Data ---")
    ...
    for model_dir in EXPERT_MODEL_DIRS:
        ...
        if model_name in X_meta_train.columns:
            ...
            if expert_probs is not None: meta_test_preds[model_name] = expert_probs
    ...
    output_df = pd.DataFrame({'DELLabel': ensemble_probs})
    if id_col_name and len(all_identifiers) == len(output_df):
        output_df[id_col_name] = all_identifiers
    ...
    print(f"\n--- Stacked Prediction Complete ---")
    print(f"Results saved to: {OUTPUT_PATH}")

if __name__ == "__main__":
    run_stacked_prediction()